...
- **API attachment handling fix**: `/api/v1/chat`, `/api/v1/sessions/:id/messages`, and the streaming/session-backed chat routes now turn `fileIds` into actual usable model context instead of just filename metadata. Text-like files are inlined into prompt context, PDFs are text-extracted when possible, and common image formats such as PNG/JPEG/WebP/GIF are passed through as inspectable attachments/context.
...
- Upload a small text file through `/api/v1/files/upload`, send it back through `fileIds`, and confirm the model can quote its contents.
- Upload a PDF through `/api/v1/files/upload`, send it back through `fileIds`, and confirm the model can answer from extracted PDF text.
- Upload a PNG or JPEG through `/api/v1/files/upload`, send it back through `fileIds`, and confirm the model can identify or describe the image.
